...
static inline void import_greenlet(void) {
  if (!init) {
    PyGreenlet_Import();
...
PyObject *greenlet_throw(PyObject *g, PyObject *typ, PyObject *val,
                         PyObject *tb) {
  import_greenlet();
...
int greenlet_dead(PyObject *g) {
  if (PyGreenlet_ACTIVE((PyGreenlet *)g) ||
      !PyGreenlet_STARTED((PyGreenlet *)g)) {
    return 0;
  } else {
    return 1;
  }
...
int greenlet_check(PyObject *g) { return PyGreenlet_Check((PyGreenlet *)g); }
