...
static void read_callback(picoev_loop *loop, int fd, int events, void *cb_arg);

#ifndef WITH_GREENLET
static void write_callback(picoev_loop *loop, int fd, int events, void *cb_arg);
#endif

static void kill_callback(picoev_loop *loop, int fd, int events, void *cb_arg);

static void trampoline_callback(picoev_loop *loop, int fd, int events,
                                void *cb_arg);

static PyObject *internal_schedule_call(int seconds, PyObject *cb,
                                        PyObject *args, PyObject *kwargs,
                                        PyObject *greenlet);

static int prepare_call_wsgi(client_t *client);

static void call_wsgi_handler(client_t *client);

static int check_status_code(client_t *client);

static pending_queue_t *init_pendings(void) {
...
static int realloc_pendings(void) {
  ...
  if (pendings->size >= pendings->max) {
    ...
    max = pendings->max * 2;
    new_heap =
        (TimerObject **)realloc(pendings->q, sizeof(TimerObject *) * max);
    if (new_heap == NULL) {
...
static void destroy_pendings(void) {
  int i = 0, len;
...
static void client_t_list_fill(void) {
  client_t *client;
...
static void client_t_list_clear(void) {
  client_t *op;
...
static client_t *alloc_client_t(void) {
  client_t *client;
  if (client_numfree) {
...
static void dealloc_client(client_t *client) {
...
static client_t *new_client_t(int client_fd, char *remote_addr,
...
static void set_log_value(client_t *client, PyObject *environ,
                          uintptr_t delta_msec) {
...
static void close_client(client_t *client) {
  ...
  DEBUG("start close client:%p fd:%d status_code %d", client, client->fd,
        client->status_code);
  ...
  if (!client->keep_alive) {
    ...
    new_client =
        new_client_t(client->fd, client->remote_addr, client->remote_port);
    new_client->keep_alive = 1;
    init_parser(new_client, server_name, server_port);
    ret = picoev_add(main_loop, new_client->fd, PICOEV_READ, keep_alive_timeout,
                     read_callback, (void *)new_client);
    if (ret == 0) {
      activecnt++;
    }
  }
...
static inline void kill_server(int timeout) {
  int listen_sock = 0;
  ...
  while ((item = PyIter_Next(iter))) {
    ...
    if (PyInt_Check(item)) {
      ...
      if (!set_callback) {
        ...
        if (timeout > 0) {
          // set timeout
          (void)picoev_add(main_loop, listen_sock, PICOEV_TIMEOUT, timeout,
                           kill_callback, NULL);
        } else {
          (void)picoev_add(main_loop, listen_sock, PICOEV_TIMEOUT, 1,
                           kill_callback, NULL);
        }
        set_callback = 1;
      }
...
static inline void set_current_request(client_t *client) {
  request *req;
  req = shift_request(client->request_queue);
  client->current_req = req;
}

static void set_bad_request_code(client_t *client, int status_code) {
  request *req;
...
static PyObject *app_handler(PyObject *self, PyObject *args) {
  int ret, active;
  PyObject *wsgi_args = NULL, *start = NULL, *current = NULL, *parent = NULL,
           *res = NULL;
  PyObject *env = NULL;
  ...
  if (res && res == Py_None) {
    PyErr_SetString(PyExc_Exception,
                    "response must be a iter or sequence object");
    goto error;
  }
  ...
  while (status != STATUS_OK) {
    if (status == STATUS_ERROR) {
      ...
      ret = picoev_add(main_loop, client->fd, PICOEV_WRITE, 300,
                       trampoline_callback, (void *)pyclient);
      if ((ret == 0 && !active)) {
        activecnt++;
      }
  ...
  switch (status) {
    ...
    case STATUS_SUSPEND:
      ...
      // set callback
      active = picoev_is_active(main_loop, client->fd);
      ret = picoev_add(main_loop, client->fd, PICOEV_WRITE, 300, write_callback,
                       (void *)pyclient);
      if ((ret == 0 && !active)) {
        activecnt++;
      }
...
static PyMethodDef app_handler_def = {"_app_handler", (PyCFunction)app_handler,
                                      METH_VARARGS, 0};

static PyObject *get_app_handler(void) {
  if (app_handler_func == NULL) {
    app_handler_func =
        PyCFunction_NewEx(&app_handler_def, (PyObject *)NULL, NULL);
  }
...
#ifdef WITH_GREENLET
static void resume_greenlet(PyObject *greenlet) {
  PyObject *res = NULL;
...
static void resume_wsgi_handler(ClientObject *pyclient) {
  PyObject *res = NULL;
  PyObject *err_type, *err_val, *err_tb;
  client_t *old_client;
...
#ifdef WITH_GREENLET
static void timeout_error_callback(picoev_loop *loop, int fd, int events,
                                   void *cb_arg) {
  ClientObject *pyclient = (ClientObject *)(cb_arg);
  client_t *client = pyclient->client;

  if ((events & PICOEV_TIMEOUT) != 0) {
    DEBUG("timeout_error_callback pyclient:%p client:%p fd:%d", pyclient,
          pyclient->client, pyclient->client->fd);
...
static void timeout_callback(picoev_loop *loop, int fd, int events,
                             void *cb_arg) {
  ClientObject *pyclient = (ClientObject *)(cb_arg);
  client_t *client = pyclient->client;

  if ((events & PICOEV_TIMEOUT) != 0) {
    DEBUG("timeout_callback pyclient:%p client:%p fd:%d", pyclient,
          pyclient->client, pyclient->client->fd);
    // next intval 30sec
...
static void trampoline_callback(picoev_loop *loop, int fd, int events,
  ...
  YDEBUG("call trampoline_callback fd:%d event:%d cb_arg:%p", fd, events,
         cb_arg);
  o = (PyObject *)cb_arg;
...
#ifndef WITH_GREENLET
static void write_callback(picoev_loop *loop, int fd, int events,
                           void *cb_arg) {
  ClientObject *pyclient = (ClientObject *)cb_arg;
  client_t *client = pyclient->client;
  int ret;
...
static int check_http_expect(client_t *client) {
  PyObject *c = NULL;
  char *val = NULL;
  int ret;
  request *req = client->current_req;
...
#ifdef PY3
static int set_input_file(client_t *client) {
  PyObject *input;
  int fd;
  request *req = client->current_req;
...
static int set_input_object(client_t *client) {
  PyObject *input = NULL;
  request *req = client->current_req;
...
static int set_read_error(client_t *client, int status_code) {
  client->keep_alive = 0;
  if (status_code == 0) {
    // bad request
    status_code = 400;
...
static int read_timeout(int fd, client_t *client) {
  RDEBUG("** read timeout fd:%d", fd);
  // timeout
  return set_read_error(client, 408);
}

static int compare_key(PyObject *env, char *key, char *compare) {
  int ret = -1;
  char *val = NULL;
...
static int check_websocket(PyObject *env) {
...
static int parse_new_protocol(request *req, char *buf, ssize_t readed,
                              int nread) {
  PyObject *env, *c;
  char *val = NULL;
...
static int parse_http_request(int fd, client_t *client, char *buf, ssize_t r) {
  int nread = 0;
  request *req = NULL;
...
static int read_request(picoev_loop *loop, int fd, client_t *client,
                        char call_time_update) {
  ...
  Py_BEGIN_ALLOW_THREADS r = read(client->fd, buf, sizeof(buf));
  Py_END_ALLOW_THREADS switch (r) {
    case 0:
...
static void accept_callback(picoev_loop *loop, int fd, int events,
                            void *cb_arg) {
  int client_fd, ret;
  client_t *client;
  ...
  if ((events & PICOEV_TIMEOUT) != 0) {
    ...
    for (i = 0; i < 8; ++i) {
      ...
      client_fd = accept4(fd, (struct sockaddr *)&client_addr, &client_len,
      ...
      if (client_fd != -1) {
        ...
        if (finish == 1) {
          ...
          ret = picoev_add(loop, client_fd, PICOEV_READ, keep_alive_timeout,
                           read_callback, (void *)client);
          if (ret == 0) {
            activecnt++;
          }
        }
...
static void setup_server_env(void) {
...
static int inet_listen(void) {
  ...
  for (p = servinfo; p != NULL; p = p->ai_next) {
    if ((listen_sock = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) ==
        -1) {
      // perror("server: socket");
      continue;
    }

    if (setsockopt(listen_sock, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(int)) ==
        -1) {
      close(listen_sock);
      PyErr_SetFromErrno(PyExc_IOError);
      return -1;
    ...
    Py_BEGIN_ALLOW_THREADS res = bind(listen_sock, p->ai_addr, p->ai_addrlen);
  ...
  // BACKLOG
  Py_BEGIN_ALLOW_THREADS res = listen(listen_sock, backlog);
  Py_END_ALLOW_THREADS if (res == -1) {
    close(listen_sock);
    PyErr_SetFromErrno(PyExc_IOError);
    return -1;
...
static int check_unix_sockpath(char *sock_name) {
...
static int unix_listen(char *sock_name, int len) {
  int listen_sock = 0;
  int flag = 1;
  int res;
  ...
  Py_BEGIN_ALLOW_THREADS res =
      bind(listen_sock, (struct sockaddr *)&saddr, sizeof(saddr));
  Py_END_ALLOW_THREADS if (res == -1) {
    close(listen_sock);
    PyErr_SetFromErrno(PyExc_IOError);
    return -1;
...
static PyObject *set_listen_socket(PyObject *temp) {
...
static PyObject *meinheld_listen(PyObject *self, PyObject *args,
                                 PyObject *kwds) {
  PyObject *o = NULL;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|OO:listen", kwlist, &o,
                                   &sock_fd)) {
    return NULL;
  }
...
static void sigpipe_cb(int signum) { DEBUG("call SIGPIPE"); }

static PyObject *meinheld_access_log(PyObject *self, PyObject *args) {
  PyObject *o = NULL;
  PyObject *func = NULL;
...
static PyObject *meinheld_error_log(PyObject *self, PyObject *args) {
  PyObject *o = NULL;
  PyObject *func = NULL;
...
static PyObject *meinheld_stop(PyObject *self, PyObject *args, PyObject *kwds) {
  int timeout = 0;

  static char *kwlist[] = {"timeout", 0};

  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|i:timeout", kwlist,
                                   &timeout)) {
    return NULL;
  }
  kill_server(timeout);
...
static inline int fire_pendings(void) {
  int ret = 1;
  TimerObject *timer = NULL;
  pending_queue_t *pendings = g_pendings;
...
static inline int fire_timers(void) {
  TimerObject *timer;
  int ret = 1;
  heapq_t *q = g_timers;
...
static int listen_all_sockets(void) {
...
#endif
      setup_listen_sock(listen_sock);
      ret = picoev_add(main_loop, listen_sock, PICOEV_READ, ACCEPT_TIMEOUT_SECS,
...
static int close_all_sockets(void) {
  PyObject *iter = NULL, *item = NULL;
  int listen_sock = 0;
...
static PyObject *meinheld_run_loop(PyObject *self, PyObject *args,
                                   PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|i:run", kwlist, &wsgi_app,
                                   &silent)) {
    return NULL;
  }
...
PyObject *meinheld_set_keepalive(PyObject *self, PyObject *args) {
  int on;
  if (!PyArg_ParseTuple(args, "i", &on)) return NULL;
  if (on < 0) {
...
PyObject *meinheld_get_keepalive(PyObject *self, PyObject *args) {
  return Py_BuildValue("i", is_keep_alive);
}

PyObject *meinheld_set_backlog(PyObject *self, PyObject *args) {
  int temp;
  if (!PyArg_ParseTuple(args, "i", &temp)) return NULL;
  if (temp <= 0) {
...
PyObject *meinheld_get_backlog(PyObject *self, PyObject *args) {
  return Py_BuildValue("i", backlog);
}

PyObject *meinheld_set_picoev_max_fd(PyObject *self, PyObject *args) {
  int temp;
  if (!PyArg_ParseTuple(args, "i", &temp)) return NULL;
  if (temp <= 0) {
...
PyObject *meinheld_get_picoev_max_fd(PyObject *self, PyObject *args) {
  return Py_BuildValue("i", max_fd);
}

PyObject *meinheld_set_max_content_length(PyObject *self, PyObject *args) {
  int temp;
  if (!PyArg_ParseTuple(args, "i", &temp)) return NULL;
  if (temp <= 0) {
...
PyObject *meinheld_get_max_content_length(PyObject *self, PyObject *args) {
  return Py_BuildValue("i", max_content_length);
}

PyObject *meinheld_set_client_body_buffer_size(PyObject *self, PyObject *args) {
  int temp;
  if (!PyArg_ParseTuple(args, "i", &temp)) return NULL;
  if (temp <= 0) {
    PyErr_SetString(PyExc_ValueError,
                    "client_body_buffer_size value out of range ");
    return NULL;
  }
...
PyObject *meinheld_get_client_body_buffer_size(PyObject *self, PyObject *args) {
  return Py_BuildValue("i", client_body_buffer_size);
}

PyObject *meinheld_set_listen_socket(PyObject *self, PyObject *args) {
  PyObject *temp;
...
PyObject *meinheld_set_fastwatchdog(PyObject *self, PyObject *args) {
  int _fd;
  int _ppid;
  int timeout = 0;
  if (!PyArg_ParseTuple(args, "iii", &_fd, &_ppid, &timeout)) return NULL;
...
PyObject *meinheld_set_watchdog(PyObject *self, PyObject *args) {
  PyObject *temp;
  if (!PyArg_ParseTuple(args, "O:watchdog", &temp)) return NULL;
...
PyObject *meinheld_suspend_client(PyObject *self, PyObject *args) {
#ifdef WITH_GREENLET
  ...
  */
  ...
  if (client && !(pyclient->suspended)) {
    ...
    BDEBUG("meinheld_suspend_client pyclient:%p client:%p fd:%d", pyclient,
           client, client->fd);
    BDEBUG("meinheld_suspend_client active ? %d",
           picoev_is_active(main_loop, client->fd));
    active = picoev_is_active(main_loop, client->fd);
    if (timeout > 0) {
      ret = picoev_add(main_loop, client->fd, PICOEV_TIMEOUT, timeout,
                       timeout_error_callback, (void *)pyclient);
    } else {
      ret = picoev_add(main_loop, client->fd, PICOEV_TIMEOUT, 3,
                       timeout_callback, (void *)pyclient);
    }
    if ((ret == 0 && !active)) {
      activecnt++;
    }
...
PyObject *meinheld_resume_client(PyObject *self, PyObject *args) {
#ifdef WITH_GREENLET
  ...
  if (!PyArg_ParseTuple(args, "O|OO:_resume_client", &temp, &switch_args,
                        &switch_kwargs)) {
    return NULL;
  }
  ...
  if (pyclient->client && pyclient->suspended) {
    ...
    pyclient->suspended = 0;
    /* pyclient->resumed = 1; */
    DEBUG("meinheld_resume_client pyclient:%p client:%p fd:%d", pyclient,
          pyclient->client, pyclient->client->fd);
    DEBUG("meinheld_resume_client active ? %d",
          picoev_is_active(main_loop, pyclient->client->fd));
    // clear event
    active = picoev_is_active(main_loop, client->fd);
    ret = picoev_add(main_loop, client->fd, PICOEV_WRITE, 0,
                     trampoline_callback, (void *)pyclient);
    if ((ret == 0 && !active)) {
      activecnt++;
    }
...
PyObject *meinheld_cancel_wait(PyObject *self, PyObject *args) {
#ifdef WITH_GREENLET
  int fd;
...
static PyObject *meinheld_trampoline(PyObject *self, PyObject *args,
                                     PyObject *kwargs) {
#ifdef WITH_GREENLET
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "i|OOi:trampoline", keywords,
                                   &fd, &read, &write, &timeout)) {
    return NULL;
  }
  ...
  if (pyclient != NULL && pyclient->greenlet == current) {
    ...
    ret = picoev_add(main_loop, fd, event, timeout, trampoline_callback,
                     (void *)pyclient);
    if ((ret == 0 && !active)) {
      activecnt++;
    }
    ...
    current = pyclient->greenlet;
    parent = greenlet_getparent(current);
    YDEBUG("trampoline fd:%d event:%d current:%p parent:%p cb_arg:%p", fd,
           event, current, parent, pyclient);
    ...
    active = picoev_is_active(main_loop, fd);
    ret =
        picoev_add(main_loop, fd, event, timeout, trampoline_callback, current);
    if ((ret == 0 && !active)) {
      activecnt++;
    }
    YDEBUG("trampoline fd:%d event:%d current:%p parent:%p cb_arg:%p", fd,
           event, current, parent, current);
    /* Py_INCREF(hub_switch_value); */
    res = greenlet_switch(parent, hub_switch_value, NULL);
    return res;
  }
...
static PyObject *meinheld_spawn(PyObject *self, PyObject *args,
                                PyObject *kwargs) {
#ifdef WITH_GREENLET
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|OO:spawn", keywords, &func,
...
static PyObject *meinheld_sleep(PyObject *self, PyObject *args,
                                PyObject *kwargs) {
#ifdef WITH_GREENLET
  PyObject *current = NULL, *parent = NULL, *res = NULL;
  int sec = 0;
...
PyObject *meinheld_get_ident(PyObject *self, PyObject *args) {
#ifdef WITH_GREENLET
...
static PyObject *meinheld_schedule_call(PyObject *self, PyObject *args,
                                        PyObject *kwargs) {
  long seconds = 0, ret;
  Py_ssize_t size;
...
static PyMethodDef ServerMethods[] = {
    {"listen", (PyCFunction)meinheld_listen, METH_VARARGS | METH_KEYWORDS,
     "set host and port num"},
    {"set_access_logger", meinheld_access_log, METH_VARARGS,
     "set access logger function."},
    {"set_error_logger", meinheld_error_log, METH_VARARGS,
     "set error logger function."},

    {"set_keepalive", meinheld_set_keepalive, METH_VARARGS,
     "set keep-alive support. value set timeout sec. default 0. (disable "
     "keep-alive)"},
    {"get_keepalive", meinheld_get_keepalive, METH_VARARGS,
     "return keep-alive support."},

    {"set_max_content_length", meinheld_set_max_content_length, METH_VARARGS,
     "set max_content_length"},
    {"get_max_content_length", meinheld_get_max_content_length, METH_VARARGS,
     "return max_content_length"},

    {"set_client_body_buffer_size", meinheld_set_client_body_buffer_size,
     METH_VARARGS, "set client_body_buffer_size"},
    {"get_client_body_buffer_size", meinheld_get_client_body_buffer_size,
     METH_VARARGS, "return client_body_buffer_size"},
    ...
    {"set_picoev_max_fd", meinheld_set_picoev_max_fd, METH_VARARGS,
     "set picoev max fd size"},
    {"get_picoev_max_fd", meinheld_get_picoev_max_fd, METH_VARARGS,
     "return picoev max fd size"},

    /* {"set_process_name", meinheld_set_process_name, METH_VARARGS, "set
       process name"}, */
    {"stop", (PyCFunction)meinheld_stop, METH_VARARGS | METH_KEYWORDS,
     "stop main loop"},
    {"shutdown", (PyCFunction)meinheld_stop, METH_VARARGS | METH_KEYWORDS,
     "stop main loop "},

    {"schedule_call", (PyCFunction)meinheld_schedule_call,
    ...
    // support gunicorn
    {"set_listen_socket", meinheld_set_listen_socket, METH_VARARGS,
     "set listen_sock"},
    {"set_watchdog", meinheld_set_watchdog, METH_VARARGS, "set watchdog"},
    {"set_fastwatchdog", meinheld_set_fastwatchdog, METH_VARARGS,
     "set watchdog"},
    {"run", (PyCFunction)meinheld_run_loop, METH_VARARGS | METH_KEYWORDS,
     "set wsgi app, run the main loop"},
    ...
    {"trampoline", (PyCFunction)meinheld_trampoline,
     METH_VARARGS | METH_KEYWORDS, "trampoline"},
...
static struct PyModuleDef server_module_def = {
    PyModuleDef_HEAD_INIT, MODULE_NAME, NULL, -1, ServerMethods,
};
...
#endif
{
  ...
  timeout_error =
      PyErr_NewException("meinheld.server.timeout", PyExc_IOError, NULL);
