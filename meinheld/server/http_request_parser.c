 ...
 * to complete the URL. Note, however, that HTTP_HOST, if present, should be
 * used in preference to SERVER_NAME for reconstructing the request URL. See the
 * URL Reconstruction section below for more detail. SERVER_NAME and SERVER_PORT
 * can never be empty strings, and so are always required.
 *
 * SERVER_PROTOCOL
 *   The version of the protocol the client used to send the request.
 *   Typically this will be something like "HTTP/1.0" or "HTTP/1.1" and may be
 * used by the application to determine how to treat any HTTP request headers.
 *   (This variable should probably be called REQUEST_PROTOCOL, since it denotes
 * the protocol used in the request, and is not necessarily the protocol that
 * will be used in the server's response. However, for compatibility with CGI we
...
static http_parser *alloc_parser(void) {
  http_parser *p;
  if (numfree) {
    p = http_parser_free_list[--numfree];
...
static PyObject *concat_bytes(PyObject *o, const char *buf, size_t len) {
  PyObject *ret;
  size_t l;
  char *dest, *origin;
...
static PyObject *concat_string(PyObject *o, const char *buf, size_t len) {
#ifdef PY3
...
static int replace_env_key(PyObject *dict, PyObject *old_key,
                           PyObject *new_key) {
  int ret = 1;
...
static int set_query(PyObject *env, char *buf, int len) {
  int c, ret, slen = 0;
  char *s0;
  PyObject *obj;
#ifdef PY3
...
static int set_path(PyObject *env, char *buf, int len) {
  int c, c1, slen;
  char *s0, *t;
  PyObject *obj;
...
static PyObject *get_http_header_key(const char *s, int len) {
  PyObject *obj;
  char *dest;
  char c;
...
static void key_upper(char *s, const char *key, size_t len) {
  size_t i = 0;
  int c;
  for (i = 0; i < len; i++) {
    c = key[i];
...
static int write_body2file(request *req, const char *buffer,
                           size_t buffer_len) {
...
static int write_body2mem(request *req, const char *buf, size_t buf_len) {
...
static int write_body(request *req, const char *buffer, size_t buffer_len) {
...
static client_t *get_client(http_parser *p) { return (client_t *)p->data; }

static request *get_current_request(http_parser *p) {
...
static int add_header(request *req) {
...
int headers_complete_cb(http_parser *p) {
  ...
  if (p->content_length != ULLONG_MAX) {
    ...
    if (max_content_length < content_length) {
      RDEBUG("max_content_length over %d/%d", (int)content_length,
...
static PyMethodDef method = {"file_wrapper", (PyCFunction)file_wrapper,
                             METH_VARARGS, 0};
...
int parser_finish(client_t *cli) { return cli->complete; }
