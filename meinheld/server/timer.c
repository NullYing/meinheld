...
TimerObject *TimerObject_new(long seconds, PyObject *callback, PyObject *args,
  ...
  // DEBUG("args seconds:%ld callback:%p args:%p kwargs:%p", seconds, callback,
  // args, kwargs);
...
static int TimerObject_clear(TimerObject *self) {
  GDEBUG("self:%p", self);
  Py_CLEAR(self->args);
...
static int TimerObject_traverse(TimerObject *self, visitproc visit, void *arg) {
  GDEBUG("self:%p", self);
  Py_VISIT(self->args);
...
static PyMethodDef TimerObject_methods[] = {
    {"cancel", (PyCFunction)TimerObject_cancel, METH_NOARGS, 0}, {NULL, NULL}};
